...
namespace ehsim {
    simul_timer::simul_timer() {
        system_frequency = 0;
    }

    simul_timer::simul_timer(uint64_t _system_frequency) {
        system_frequency = _system_frequency;
    }

    std::chrono::nanoseconds simul_timer::current_system_time() {
        return system_time;
    }

    void simul_timer::inactive_tick() {
        system_time += 1ms;
        last_ms_mark += 1ms;
        elapsed_time += 1ms;
    }

    void simul_timer::active_tick(uint64_t cycles) {
    ...
    bool simul_timer::harvest_while_active() {
    ...
    std::chrono::nanoseconds simul_timer::get_elapsed_time() {
