...
for line in lines:
	...
	if (line != lines[0] and line != lines[1]):
		...
		task_histories[entries[0]].append(entries[2])	
...
def bar_graph(failure_frequency_dict, pc, axs, axs_num):
	labels = [];
	frequency = [];
	...
	axs[axs_num].bar(labels,frequency, align='center');
	axs[axs_num].set_title("Task PC: " + str(pc), fontsize = 14)
	
	axs[axs_num].set_xlabel('Iteration # where task failed',fontsize=12)
fig, axs = plt.subplots(len(iteration_failure_frequencies))
